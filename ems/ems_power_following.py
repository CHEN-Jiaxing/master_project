...
for i in range(len(p_bus_list)):
    ...
    '''
    p_bus = p_bus_list[i]
    # power following energy manegement strategy
    if SOC < SOC_low_limit:
        p_fc = p_bus * 1.2
        p_bat = p_bus - p_fc
    elif SOC <= SOC_high_limit and SOC >= SOC_low_limit:
        p_fc = p_bus * 0.8
        p_bat = p_bus - p_fc
    else:
        p_fc = p_bus * 0.4
...
print(SOC_list[-1])
'''
np.save("./fcon_pwfl.npy", fuel_consumption_list)
np.save("./deg_pwfl.npy", fc_deg_list)
np.save("./eff_pwfl.npy", fc_eff_list)
np.save("soc_pwfl", SOC_list)
np.save("pfc_pwfl", p_fc_list)
np.save("pbat_pwfl", p_bat_list)
'''
print("=====END=====")